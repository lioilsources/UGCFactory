...
MIA od 2026-09-15 na Sparku běží (oprava prostředí v plánu §13) a u fotek
celé postavy deformuje líp, u brnění s pláštěm a oříznutých postav hůř. Proto
`FC_RIG=auto` udělá oba rigy, nasadí na ně první klip postavy a nechá ten, co
se míň trhá; `rig_report.json` nese `rig_choice` a `rig_scores`. Když MIA
selže nebo do `FC_RIG_MIA_TIMEOUT` nedoběhne (sdílená fronta ComfyUI), zůstane
šablona. `template` a `comfy` vynutí jednu cestu.
